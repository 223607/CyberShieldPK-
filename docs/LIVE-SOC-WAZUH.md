...
- Historical alerts are queried from `wazuh-alerts-*` through the Wazuh Indexer
- The browser receives newly appended `alerts.json` events through SSE
- No synthetic alert is generated by the SOC UI
- If Wazuh is unavailable, the UI reports offline instead of pretending demo data is live
...
## 7. Verify Wazuh before starting CyberShieldPK
...
## 8. Connect a Windows 11 endpoint
...
3. Historical alert queries are already implemented through the Wazuh Indexer; the next layer is filtering, pagination and analyst search.
